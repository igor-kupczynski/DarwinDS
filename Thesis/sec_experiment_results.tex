...
solution is robust with respect to algorithm parameters, inconsistencies in
Decision Maker's choices and finally to the uncertainty in the problem
definition. Nevertheless the analyst should be careful and investigate the
problem instead of using the method blindly. There are some problems which are
hard for DARWIN to solve (e.g. DTLZ1). Even for these problems satisfactory
results can be achieved, however one has to fine-tune the parameters and
carefully observe their impact on the result.
