DARWIN is a new method and according to knowledge of the author it hasn't been
...
reasonable default values. To make results repeatable the DM is mocked. Noise
in his or her decisions is simulated. Unless uncertainty is involved
comparisons to exact optimal solution are provided. In tests involving
uncertainty results are compared to supposed utility function optimisation.
...
essential to mock the DM. Mocking the decision make guarantees repeatable
...
Unfortunately real decision maker, being a human isn't as predictive and
...
used to solve multi-objective optimisation problems even when the problem
formulation does contain uncertainty in form of intervals of possible values.

At the moment of writing according to author's knowledge there was no working
implementation of the DARWIN method. So the first task was to analyse behavior
of the algorithm on simple two criteria problem without uncertainty. First
evaluations allowed the author to learn the properties and to get to know the
behaviour of the algorithm.

Having confidence that the DARWIN works it was possible to advance to the next
task --- to evaluate the performance of richer set of the problem. It was
decided to start without allowing uncertainty. Rationale for this behaviour
was to prove that algorithm can solve a category of problems with good overall
performance. Ignoring the uncertainty it was possible to compare the DARWIN
results with optimal ones, calculated by the linear programming solver.

There are lot of parameters in the algorithm itself, so logical next step was
to check the importance of the parameters. Experiments performed so far
assumed that the decision maker is infallible, consistent and acts according
to a well defined utility function. It is not the case in real-life situations
so the impact of noise in DM's decisions was measured.

Knowing the characteristics, performance and impact of the parameters and the
noise one can check if the algorithm will similar characteristics on problems
with interval coefficients.

To conclude the results --- the DARWIN method can handle very well MMO
problems. A resulting solution is robust with respect to algorithm parameters,
decision-maker's noise and finally to the uncertainty in problem
definition. Nevertheless analyst should be care and research the problem
instead of blindly using the method. There are some problems which are hard
for the DARWIN to solve (e.g. DTLZ1). Even if the problem is appropriate and
the method promises good results one have to fine-tune the parameters.
