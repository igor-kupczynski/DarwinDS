...
Optimisation (IMO, see~[inref]) and Evolutionary Multiobjective Optimization
(EMO, see~[inref]).

In the IMO paradigm one wants to elicit Decision Maker (DM) preferences by
involving her in the process. This is done by a systematic dialog with the
...
Human factor is another thing to consider. The DM is a human being and thus her
...
user do not need to have an expert knowledge in the decision support field.
...
uncertainty scenarios. DARWIN allows to give the coefficients in form of
intervals.
...
Set of all the problem's coefficients given as intervals and fixed on one of
possible values will be called scenario of uncertainty (see
...
scenarios.
 ...
 distribution of a random variable. Instead of presenting all the results the
 method calculates meaningful quantiles of results distribution for each
...
DRSA will be applied in IMO process. After selection of ``good'' solutions
from the provided set decision rules are induced to store the
