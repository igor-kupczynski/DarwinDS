...
def problem_decription_darwin(items, cri, coefficient, ut_weights):
    ...
    for idx in xrange(len(items)):
        out.append("var[0.0, 1.0000001] x%d;" % idx)
    out.append("")
    values = []
    ...
    for idx in xrange(len(items)):
        out.append("const%d : x%d <= 1;" % (idx, idx))
    out.append("")
...
def main():
    ...
    "python %s knapsack_continous_10 10 2 0 100.0 0.25 1.0 -1.0") % (sys.argv[0], sys.argv[0])
