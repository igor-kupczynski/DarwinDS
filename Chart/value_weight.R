...
prep.data <- function(outerIdx, x) {
  inner <- subset(x, outer==outerIdx & generation %in% c(0,10,20,30,40,50,60),
                  select=c("generation", "value1_50.0", "value2_50.0"))
  inner$generation <- factor(inner$generation)
  inner
}

gen.plot <- function(outerIdx, data) {
  c <- ggplot(data, aes(value1_50.0, value2_50.0, colour=generation))
  c <- c + geom_point()
  c <- c + opts(title=paste("Value1Value2, outer=", outerIdx, sep=""))
  c
}
